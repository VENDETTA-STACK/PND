...
router.post('/signup',async function(req,res,next){
  ...
  try{
    ...
    if(existCustomer.length==1){
      res.status(200)
      .json({Message:"Customer Already Registered!",Data:0,IsSuccess:true});
    }else{
...
router.post('/signin',async function(req,res,next){
  ...
  try{
    let existCustomer = await customerSchema.find({mobileNo:mobileNo,isVerified:true,isActive:true});
    if(existCustomer.length == 1){
      res.status(200)
      .json({Message:"Customer Found!",Data:existCustomer,IsSuccess:true});
    }else{
      res.status(200)
      .json({Message:"Customer Not Found!",Data:existCustomer,IsSuccess:true});
    }
  }catch(err){
    res.status(500)
    .json({Message:err.message,Data:0,IsSuccess:false});
...
router.post('/updateprofile',uploadpic.single('profilepic'),async function(req,res,next){
  const file = req.file;
  const {id,name,email} = req.body;
  try{
    if(file!=undefined){
      let existCustomer = await customerSchema.findByIdAndUpdate(id,{name:name,email:email,image:file.path});
      ...
      let existCustomer = await customerSchema.findByIdAndUpdate(id,{name:name,email:email});
      if(existCustomer!=null)
        res.status(200).json({Message:"Profile Updated!",Data:1,IsSuccess:true});
      else
        res.status(200).json({Message:"Profile Not Updated!",Data:0,IsSuccess:true});
